...
\begin{definition}[Observation-$o$]
...
\begin{definition}[Dataset-$q$]
It is the tuple $\{O,m,D,A,T\}$, where $O$ is a set of observations for only one measure $m$ that is described under 
a set of dimensions, $D$, and a set of annotations, 
\end{definition}


\begin{definition}[Aggregated dataset-$aq$]
It is a composed dataset which set of observations $O$ is computed through 
the aggregation of a set of existing datasets $Q_i$ through an OWA operator $p$.
\end{definition}

As a necessary condition for the computation proces, an aggregated dataset $aq$ defined by means of the set of dimensions $D_{aq}$ can be computed iif 
$\forall q_j \in Q_i: D_{aq} \subseteq D_{q_j}$.
...
\begin{itemize}
 ...
 the same context.
 \item An index $i$ and a component $c$ are aggregated datasets. (FIXME: sure?)
 \item An indicator $in$ can be both dataset or aggregated dataset.
 \item All elements that comprises the definitions must be uniquely identified. 
\end{itemize}
