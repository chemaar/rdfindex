...
It is a tuple \textbf{$\{v,m,s\}$}, where \textbf{$v$} is a numerical value for the measure \textbf{$m$} with an status \textbf{$s$} that belongs to 
only one dataset of observations \textbf{$O$}. 
\end{block}


\begin{exampleblock}{Dataset-$q$}
It is a tuple \textbf{$\{O,m,D,A,T\}$} where \textbf{$O$} is a set of observations for only one measure \textbf{$m$} that is described under 
a set of dimensions \textbf{$D$} and a set of annotations \textbf{$A$}. Additionally, some attributes can be defined in the set \textbf{$T$} for structure enrichment. 
\end{exampleblock}


\begin{alertblock}{Aggregated dataset-$aq$}
It is an aggregation of \textbf{$n$} datasets \textbf{$q_i$} (identified by the set \textbf{$Q$}) which set of observations \textbf{$O$} is derivate by applying 
an OWA operator \textbf{$p$} to the observations \textbf{$O_{q_i}$}. 
\end{alertblock}
...
An aggregated dataset \textbf{$aq$} defined by means of the set of dimensions \textbf{$D_{aq}$} can be computed iif 
\textbf{$\forall q_j \in Q: D_{aq} \subseteq D_{q_j}$}. Furthermore the OWA operator \textbf{$p$} can only aggregate values belonging to the same measure \textbf{$m$}.
\end{exampleblock}


\begin{itemize}
 \item The set of dimensions \textbf{$D$}, annotations \textbf{$A$} and attributes \textbf{$T$} for a given dataset \textbf{$Q$} is always the same with the aim of describing all observations under 
 the same context.
 \item An index \textbf{$i$} and a component \textbf{$c$} are aggregated datasets. Nevertheless this restriction is relaxed if observations can be directly mapped to 
 these elements without any computation process.
 \item An indicator \textbf{$in$} can be both dataset or aggregated dataset.
...
\frame{
  \frametitle{Evolution: joint effort between the RDFIndex and Computex}
  \scriptsize
